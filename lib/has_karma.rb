...
module Karma #:nodoc:
  ...
  module ClassMethods
    def has_karma(rateable_type)
      self.class_eval <<-RUBY
        def karma_rateable
          #{rateable_type.to_s.classify}
        end
      RUBY
      include Karma::InstanceMethods
      extend Karma::SingletonMethods
      if self.karmatic_objects.nil?
        self.karmatic_objects = [eval(rateable_type.to_s.classify)]
      else
        self.karmatic_objects.push(eval(rateable_type.to_s.classify))
      end
    end
  end
